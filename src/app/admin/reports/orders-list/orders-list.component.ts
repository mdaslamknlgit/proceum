...
export class OrdersReportComponent implements OnInit {
    ...
    applyFilters(){
        ...
        let param = { url: 'orders-list-report',"offset": this.page, "limit": this.pageSize, duration:this.duration, country_id:this.country_id, "sort_by": this.sort_by,"search_txt":this.search_txt,"from_date":this.fromDate,"to_date":this.toDate};
        this.http.post(param).subscribe((res) => {
          this.dataSource = new MatTableDataSource(res['data']);
          this.dataSource.sort = this.sort;
          this.num_rows = res['total_orders'];

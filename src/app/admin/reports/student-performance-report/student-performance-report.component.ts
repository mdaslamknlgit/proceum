...
export class StudentPerformanceReportComponent implements OnInit {
  displayedColumns: string[] = ['s_no', 'student_name', 'student_email', 'uid', 'last_login', 'notes_accessed', 'videos_watched', 'zoom_class','assessment_attended','subject_knowledge'];
  ...
  @ViewChild(MatSort) sort: MatSort;
    ...
    public subject_know_popup = false;
    public subjectknowledge;

  constructor(private http: CommonService, private toster: ToastrService,public translate: TranslateService) { }
  ...
  public doFilter() {
    ...
    this.totalSize = 0;
    if(this.student_name != '' && this.organization_list_id == ''){
    ...
    let param = {
    url: 'reports/student-performance-list',
    offset: this.page,
    limit: this.pageSize,
    role: '2',
  ...
  openSubjectKnowledgeModel(data: any){
    this.subject_know_popup = true;
    let user_id = data.id;
    let partnter_id = this.organization_list_id;
    let list_type_id = this.organization_type_id;
    let param = { url: 'subject-knowledge', user_id:user_id, list_type_id:list_type_id, partnter_id:partnter_id};
    this.http.post(param).subscribe((res) => {
      if (res['error'] == false) {
